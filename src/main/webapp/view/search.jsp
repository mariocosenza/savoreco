...
<search>
    <form action="search" method="get">
        <input inputmode="search" autocomplete="off" placeholder="Inserisci nome del ristorante" name="byName"
               type="search"
...
<div class="mobileSelector">
    ...
    <select id="selectSearch" class="mobile classic" onclick=toggleSelect(this)>
        ...
        <c:forEach items="${requestScope.restaurants.stream().map(r -> r.category).sorted().distinct().toList()}"
                   var="category">
...
<main>
    <aside>
        ...
        <c:forEach items="${requestScope.restaurants.stream().map(r -> r.category).sorted().distinct().toList()}"
                   var="category">
            <div class="filter" id='searchCategory${category.replaceAll("\\s+","")}'
                 style="border-color: var(--md-sys-color-secondary-container)"
                 onclick="toggleCategory(this,'${category.replaceAll("\\s+","")}')">
                <c:out value="${category}"/>
            </div>
        </c:forEach>
                    ...
                    <div class="result resultCategory${resto.category.replaceAll("\\s+","")}">
                        ...
                        <div class="infoResto">
                            ...
                            <h2><img class="svgLogo" src="../assets/icons/placeholder-pin-svgrepo-com.svg"
                                     alt="Distanza"> <fmt:formatNumber value='<%=SloppyMath.haversinMeters(Double.parseDouble(request.getParameter("lat")), Double.parseDouble(request.getParameter("lon")),
                                        ((Restaurant)   pageContext.getAttribute("resto")).getAddress().getLat(), ((Restaurant)   pageContext.getAttribute("resto")).getAddress().getLon())/1000%>'
                                                                       pattern="#.#"/> km</h2>
                            <h2><c:out value="${resto.category}"/></h2>
                            <h2 class="price">Costo consegna <fmt:formatNumber value="${resto.deliveryCost}"
                                                                               pattern="#.#"/> €</h2>
